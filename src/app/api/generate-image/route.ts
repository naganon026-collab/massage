...
export async function POST(req: Request) {
    ...
    try {
        ...
        if (effectiveTargets.instagram) {
            jsonFormatGuide += `\n  "instagram": "【目的: 共感とファン化】絵文字を適度に使い、SNSらしい改行テンポで構成。画像の内容をテキストでも具体的に表現し、ユーザーの興味を惹く文章に。語尾は丁寧な「です・ます調」で統一。最後には必ず『ご予約・お問い合わせはプロフィールのLINEからお待ちしております（URL: ${shopLineUrl} ）』のようにLINEへの誘導文を含めること。（ハッシュタグ #リラクゼーション #${shopName} #メンズ専用 等を含む）"`;
        ...
        if (effectiveTargets.gbp) {
            ...
            jsonFormatGuide += `\n  "gbp": "【目的: 近隣検索からの来店誘致】画像から得られる情報（雰囲気や成果など）を明確に伝え、「ここなら悩みが解決できそう」と思わせる力強い文章。文章の最後は必ず予約・問合せ情報（詳細やご予約はLINEから: ${shopLineUrl} ）への自然な導線で結ぶこと。"`;
        ...
        if (effectiveTargets.portal) {
            ...
            jsonFormatGuide += `\n  "portal": "<div class=\\"seo-content\\"><h3>導入・画像に関する見出し</h3><p>本文（画像の解説と共感）</p><h4>${shopName}の強みとアピールポイント</h4><p>本文</p><div class=\\"shop-info\\"><p>📍${shopAddress}</p><p>${phoneHtml}</p><p>🕒${shopBusinessHours}</p><p>🎌${shopHolidays}</p><div style=\\"text-align:center;\\"><a href=\\"${shopLineUrl}\\" style=\\"${lineButtonStyle}\\">LINEでのご予約・ご相談はこちら</a></div></div></div> のような、HTMLタグで構造化・装飾された長文のコラム風テキスト。末尾にLINEへの誘導リンクを含めること。"`;
        ...
        if (effectiveTargets.line) {
            if (jsonFormatGuide) jsonFormatGuide += ",";
            jsonFormatGuide += `\n  "line": "【目的: LINE公式アカウントからの配信メッセージ】LINEらしい改行と絵文字を使い、友だち登録しているお客様に向けて親しみやすく語りかける文章。画像について言及しながら、今回の「伝えたいこと」を温かみのある文体で伝える。文章の最後は必ず『ご予約・お問い合わせはこちら↓\n${shopLineUrl}』のようにLINE URLへの導線で締めること。（ハッシュタグ不要、文字数200〜350字程度）"`;
