...
const HOOK_TYPES = [
    { id: "kyoukan", label: "共感型", example: "「なんとなく肩が重だるい…」そんな感覚、ありませんか？" },
    { id: "odoroki", label: "驚き型", example: "実は、毎日の「強いマッサージ」が体を傷めていたかもしれません。" },
    { id: "toikake", label: "問いかけ型", example: "あなたのその腰痛・肩こり、整体で根本から解決できるかもしれません。" },
    { id: "number", label: "数字・リスト型", example: "整体師が絶対やらないNGセルフケア3選" },
    { id: "ng", label: "NG・やりがち型", example: "やりがちだけど逆効果…体を硬くする日常の悪習慣" },
    { id: "shiranai", label: "知らなかった型", example: "99%の人が知らない、肩こり・腰痛をぶり返さない方法" },
    { id: "pro", label: "プロだけが知る型", example: "プロだけが知る、季節の変わり目にやっておくべき体のケア" },
];

const STORY_SUBJECTS = [
    { id: "customer", label: "お客様目線", note: "お客様が主人公のストーリーで語る。「〇〇でお悩みだったお客様が…」という視点。" },
    { id: "stylist", label: "セラピスト・整体師目線", note: "セラピスト・整体師自身が語りかける一人称スタイル。「今日担当させていただいたお客様が…」という温かみ。" },
...
const SEASONAL_ANGLES = [
    { id: "climate", label: "気候から入る", note: "天気・気温・湿度など体感できる季節の変化から自然に体の不調の話につなげる。" },
    ...
    { id: "worry", label: "季節特有の悩みから入る", note: "気圧変化による頭痛・冬の冷えによる肩こり・梅雨の自律神経の乱れなど、その時期ならではの体の悩みから直球で入る。" },
];
...
- 投稿の主役は施術・スタッフ・店舗の情報
- 長くなりすぎない（1〜2文以内）
- 天気から体の冷えや不調・来店への自然な流れを作る
  例：「今日は少し肌寒い一日ですね❄️
      こんな日は体が縮こまって肩こりや腰痛が出やすくなります」
`
            : `
【天気について】
...
`;
        ...
        if (outputTargets.instagram) {
            ...
            const hashtagNote = shopCity
                ? `**【必須】ハッシュタグは地域名＋悩み＋店舗名の3層構造で10〜15個。①地域層：#${shopCity.replace(/\s/g, "")} #${shopCity.replace(/\s/g, "")}整体 等 ②悩み層：今回のテーマに合わせて（#肩こり #腰痛 #首こり #眼精疲労 #むくみ 等）③店舗層：#${shopName.replace(/\s/g, "")}。汎用タグ（#根本改善 #整体院 等）も可。**`
                : `**【必須】ハッシュタグは悩み＋店舗名の2層以上で10〜15個。悩み層（#肩こり #腰痛 #首こり 等、テーマに合わせて）＋店舗層（#${shopName.replace(/\s/g, "")}）＋汎用（#根本改善 #整体院 等）。**`;
...
他のどの店にも書けない、このユーザーだけの血の通ったオリジナル文章（一次情報）に仕上げること。
---` : "";

        const systemPrompt = `# あなたの役割
あなたは整体・マッサージ業界専門のSNSマーケティングコンサルタントです。
${shopAddress}の${shopIndustry}「${shopName}」を担当し、特に20〜40代女性をターゲットにした「集客につながる投稿」を作成します。
...
同時に、どの投稿でも「根本改善」「姿勢・骨盤矯正」「丁寧なカウンセリング」を、この院や店舗の強みとして不自然にならない範囲で自然に織り込んでください。
...
2. ボディ（解決策）
   ...
   - 解決策のパートでは、必ず店舗の強み（根本改善 / 姿勢・骨盤矯正 / 丁寧なカウンセリング）に自然につなげる。
...
- **ハッシュタグ**：地域名（#〇〇市 #〇〇市整体 #〇〇市マッサージ）＋悩み（#肩こり #眼精疲労 #むくみ 等、テーマに合わせて）＋店舗名（#${shopName}）の3層構造で10〜15個。

■ GoogleMap投稿（Googleビジネスプロフィール / GBP 用）
- **200〜280字程度に短く収める**。
- 信頼感・専門性を重視した文体にする。
- **構成**：悩みの共感 → 解決策 → なぜ今来店すべきか1文 → 締め（地域名＋CTA）。
- 専門用語は、難しすぎない範囲で適度に使う（例：筋膜リリース、骨盤矯正など）。
...
- ボディ部分では「あるあるな悩み → 原因 → 解決策（店舗の強み）」を2秒ごとのテロップで展開する。
- 最後の数秒で、「なぜ今」の一言 → 「保存・フォロー・予約」を自然な言い方で促す。

---
# 絶対に守るルール
- 「根本改善」「姿勢・骨盤矯正」「丁寧なカウンセリング」を、この店舗の強みとして不自然にならない形で必ずどこかに入れる。
...
- 価格・料金（例：「もみほぐし60分5,500円」「骨盤矯正3,300円〜」）
...
- スタッフの名前・人数・役職（例：「田中セラピストが担当」「3名体制」）
- 受賞歴・資格・認定（例：「○○コンテスト受賞」「認定院」）
...
【画像分析と執筆のステップ（思考プロセス）】
1. **視覚情報の抽出**: 写っているもの（施術の様子、スタッフの手技、店内の雰囲気・清潔感、お客様の表情や姿勢など）を詳細に言語化してください。
2. **価値の言語化**: 単なる説明ではなく、「このほぐし方は深層筋まで届く当店独自の手技の結果である」「この丁寧な準備がお客様の安心感につながっている」といった、プロならではの価値に変換してください。
3. **ターゲットへの共感**: その価値を必要としている20〜40代の方が、普段どんな悩み（肩こり・腰痛・疲れがとれない・姿勢の悪さなど）を抱えているかを想像してください。

【投稿の構成ルール（厳守）】
1. **フック（悩みの共感）**: 
   - 冒頭は必ず「ターゲットの悩みや理想への寄り添い」から始めてください。
   - 例：「毎日のデスクワークで、肩や首が重だるくなっていませんか？」「頑張っている自分の体に、プロのケアをプレゼントしてみませんか。」
2. **ボディ（画像×解決策）**: 
   - 画像で見えている「証拠」を使いながら、当店がどのようにその悩みを解決したか、あるいは理想を叶えたかを伝えてください。
   - 店舗の強み（根本改善・独自の矯正技術・丁寧なカウンセリング等）を、画像の内容と絡めて自然に語ってください。
3. **今来店すべき理由**: 
...
1. フックは以下のいずれかのパターンで書く
   ・「○選」型：「現役整体師が絶対やらないNGケア3選」
   ・「疑問」型：「なぜあなたの肩こりはすぐぶり返すのか」
   ・「知らなかった」型：「99%の人が知らない湿布・マッサージの真実」
...
6. Instagramハッシュタグは地域名＋悩み＋店舗名の3層構造で15個生成する
   地域層：市区町村名（#長野市 #〇〇市整体 等）
   悩み層：テーマに紐づくタグ（#ストレッチ #肩こり解消 #体のお悩み 等）
   店舗層：#${shopName}＋季節タグ
${
                ctxB.memo?.trim()
                    ? `\n【スタッフからの補足メモ】\n${ctxB.memo}\n（このメモを投稿に自然に活かすこと）`
...
6. Instagramハッシュタグは3層構造で15個生成する
   ビッグワード  3個（#整体院 #根本改善 #ストレッチ等）
   ミドルワード  7個（施術名・お悩みに紐づくタグ）
   スモールワード 5個（地域名＋店舗名＋季節タグ）
`;
...
一人称で、指名予約への導線と#指名したい整体師を含めること。
`;
        } else if (patternId === "H") {
            userPrompt = `
以下の情報を元に、店舗の裏側・日常が伝わる投稿を作成してください。
【今日の場面】${q1 || "記載なし"}
【伝えたいメッセージ】${q2 || "記載なし"}
【届けたいお客様】${q3 || "記載なし"}
具体的な描写と#整体院の裏側を含めること。
`;
        } else {
