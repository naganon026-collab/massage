...
export async function POST(req: Request) {
    ...
    try {
        ...
        const { patternTitle, patternId, useWeather: useWeatherParam, additionalContext, q1, q2, q3, shopInfo, news, outputTargets, imageData, generatedAt } = parsed.data;
...
---
# 必須の出力構造（全媒体共通）
すべての媒体（Instagram / Google情報 / LINE / ショート動画台本）で、以下の3部構成を必ず守ってください。
...
3. CTA
   - 「保存」「フォロー」「予約」の3つのアクションを、押しつけがましくなく自然に促す。
   - 例：「あとから見返せるように、ぜひ保存しておいてくださいね」「参考になったらフォローして、明日のケアのヒントも受け取ってください」「髪のお悩みは、プロフィールのリンクからお気軽にご相談ください」など。
...
■ Google情報（Googleビジネスプロフィール / GBP 用）
- 信頼感・専門性を重視した文体にする。
...
${
            (() => {
                try {
                    const ctx = additionalContext ? (JSON.parse(additionalContext) as { memo?: string }) : {};
                    return ctx.memo?.trim()
                        ? `\n【スタッフからの補足メモ】\n${ctx.memo}\n（このメモを投稿に自然に活かすこと）`
                        : "";
                } catch {
                    return "";
...
`;
            ...
            try {
                const ctx = JSON.parse(additionalContext) as { scene?: string; message?: string; target?: string; memo?: string };
                userPrompt = `
