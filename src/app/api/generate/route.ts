...
`;
        ...
        if (outputTargets.instagram) {
            ...
            jsonFormatGuide += `\n  "instagram": "【目的: 共感とファン化】絵文字を適度に使い、SNSらしい改行テンポで、今日あったリアルなエピソードを語る文章。**短く簡潔に（150〜220字程度）**。語尾は丁寧な「です・ます調」で統一すること。**【必須】文章の最後の1〜2文に、次のCTAをそのまま含めること（言い換え禁止）：『${instagramCta}』**（ハッシュタグ #リラクゼーション #${shopName} #メンズ専用 等を含む）"`;
        ...
        if (outputTargets.line) {
            ...
            jsonFormatGuide += `\n  "line": "【目的: LINE公式アカウントからの配信メッセージ】LINEらしい改行と絵文字を使い、友だち登録しているお客様に向けて親しみやすく語りかける文章。**【必須】1文ごとに改行を入れること。1行に1文だけ。長い文は途中で改行せず、1文を短く区切ること。改行を多めにして、スマホで読みやすいよう余白を意識すること。**${lineGreetingNote}今日の施術エピソードや限定情報を温かみのある文体で簡潔に伝える。**【必須】文章の最後に、次のCTAをそのまま含めて締めること：『${lineCta}』**（ハッシュタグ不要、**文字数80〜120字程度で短く簡潔に**）"`;
