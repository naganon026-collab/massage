...
export async function POST(req: Request) {
    ...
    try {
        ...
        const { patternTitle, patternId, useWeather: useWeatherParam, additionalContext, q1, q2, q3, shopInfo, news, outputTargets, imageData, generatedAt, todaysFocus } = parsed.data;
...
`;
        ...
        // CTA文言を組み立て（ctaType+ctaValue を優先、旧 ctaText は ctaType 未設定時のみ使用）
        ...
        if (outputTargets.instagram) {
            const instagramCta = shopCtaText || `ご予約・お問い合わせはプロフィールのLINEからお待ちしております${shopLineUrl ? `（URL: ${shopLineUrl} ）` : ""}`;
            jsonFormatGuide += `\n  "instagram": "【目的: 共感とファン化】絵文字を適度に使い、SNSらしい改行テンポで、今日あったリアルなエピソードを語る文章。語尾は丁寧な「です・ます調」で統一すること。**【必須】文章の最後の1〜2文に、次のCTAをそのまま含めること（言い換え禁止）：『${instagramCta}』**（ハッシュタグ #リラクゼーション #${shopName} #メンズ専用 等を含む）"`;
            properties.instagram = { type: SchemaType.STRING };
            required.push("instagram");
        }
        if (outputTargets.gbp) {
            if (jsonFormatGuide) jsonFormatGuide += ",";
            // 住所から市区町村単位だけを抽出（例：中野市、渋谷区。「一本木」などの町名・番地は含めない）
            const shopCity = shopAddress
                .replace(/^(東京都|北海道|(?:京都|大阪)府|.{2,3}県)/, "")
                .trim()
                .replace(/^(.+?[市区町村]).*$/, "$1")
                .trim() || "";
            const gbpCta = shopCtaText || `詳細やご予約はLINEから: ${shopLineUrl}`;
            jsonFormatGuide += `\n  "gbp": "【目的: Googleマップから来店を促す】Googleマップで美容室を探しているお客様に向けた、シンプルで信頼感のある来店訴求文。【構成】①冒頭：お客様のお悩みへの共感1〜2文②本文：${shopName}だからこそできる解決策・強みを具体的に1〜2段落③**【必須】締め**：最後に必ず「${shopCity}近隣で〜をお考えの方は、ぜひ${shopName}へお気軽にご相談ください。${gbpCta}」と、この締め文をそのまま含めて終わること。【禁止事項】・教育コンテンツ・NGケア紹介・リスト形式（〇選、〇つのポイントなど）は使わない・締め文以外に地域名・住所は入れない・タイトルや見出しは入れない・冒頭に「○○で美容室をお探しの方へ」のような呼びかけ文を入れない。冒頭は必ずお客様のお悩みへの共感から始めること。・価格・割引・限定キャンペーン・特典などの情報は、入力された情報にない場合は絶対に作らない。・文章は300〜400字程度に収める。どのパターン・テーマが選ばれていても、必ずこの来店訴求の構成で書くこと。"`;
        ...
        if (outputTargets.portal) {
            ...
            const portalCtaLabel = shopCtaText || "LINEでのご予約・ご相談はこちら";
        ...
        if (outputTargets.line) {
            if (jsonFormatGuide) jsonFormatGuide += ",";
            const lineCta = shopCtaText || `ご予約・お問い合わせはこちら↓\n${shopLineUrl}`;
            jsonFormatGuide += `\n  "line": "【目的: LINE公式アカウントからの配信メッセージ】LINEらしい改行と絵文字を使い、友だち登録しているお客様に向けて親しみやすく語りかける文章。冒頭に季節の挨拶や今日のひとことを入れ、今日の施術エピソードや限定情報を温かみのある文体で伝える。**【必須】文章の最後に、次のCTAをそのまま含めて締めること：『${lineCta}』**（ハッシュタグ不要、文字数200〜350字程度）"`;
...
- 最後の cta は、${shopCtaText ? `設定されたCTA「${shopCtaText}」の趣旨を20字前後で簡潔に。` : `「LINEで予約」「プロフィールのリンクから」など、${shopLineUrl ? `LINE（${shopLineUrl}）` : "予約・問い合わせ"}への誘導を20字前後で。`}
...
CTA文：「${shopCtaText || "ご予約・お問い合わせはプロフィールのLINEからお待ちしております"}」

---
# 必須の出力構造（全媒体共通）
すべての媒体（Instagram / GoogleMap投稿 / LINE / ショート動画台本）で、以下の3部構成を必ず守ってください。

1. フック
   - 最初の1〜2行で「スクロールを止める」一文。
   - 「〇選」「NGケア」「プロだけが知る」「知らないと損」「やりがち」など、20〜30代女性の興味を引く言葉を使う。
2. ボディ
   - 「問題提起 → 原因 → 解決策」の流れを守る。
   - 解決策のパートでは、必ずサロンの強み（髪質改善 / トレンドカラー / 丁寧なカウンセリング）に自然につなげる。
   - 読者が「これ、自分のことかも」と感じるような、具体的なシーン・ひと言・感情を入れる。
3. CTA（文末に必須）
...
- ハッシュタグは10〜15個程度つける。
- ハッシュタグには「地域名＋美容院」「髪質改善」「トレンドカラー」「白髪ぼかし」「インナーカラー」などを組み合わせる。
...
- 友達に話すような、あたたかいトーン。
- 1通あたり長すぎないボリュームにまとめる。
- 冒頭で軽いあいさつ → 悩みやお知らせ → 予約・相談への一言、という流れを意識する。

■ ショート動画（shortScript 用）
- 0秒・2秒・4秒・6秒・8秒・10秒…というように、2秒ごとの字幕テキストとして読める構成にする。
- 冒頭の数秒で、フックとして「気になる一言」や「NGケア」「〇選」などを出し、視聴者の注意を引く。
- ボディ部分では「あるあるな悩み → 原因 → 解決策（サロンの強み）」を2秒ごとのテロップで展開する。
- 最後の数秒で、「保存・フォロー・予約」のいずれか（できればすべて）を自然な言い方で促す。
...
{${jsonFormatGuide}
        ...
        if (imageData) {
            userPrompt = `以下の【アップロードされた画像】の内容を分析し、${shopAddress}の${shopIndustry}「${shopName}」のオーナーとして、その画像に合った投稿文を作成してください。
...
`;
            ...
            userPrompt = `
今日の日付・季節情報をもとに、「${shopName}」にとって
今日投稿すべき最適な内容をAIが自律的に判断して生成してください。

【今日の季節・時期情報】
${seasonal.text}
${todaysFocus ? `\n【今日の切り口（最優先で使うこと）】\n${todaysFocus}\nSTEP1のお悩み選定では、必ずこの切り口に沿ったお悩みを選ぶこと。` : ""}

【店舗の強み（必ず活かすこと）】
${shopFeatures || "髪質改善・トレンドカラー・丁寧なカウンセリング"}

【生成ステップ（必ずこの順で実行）】

STEP1: 今日の時期から「最も共感されるお悩み」を1つ選ぶ
  例：3月中旬なら「新生活前にイメチェンしたいけど何が似合うか不安」

STEP2: そのお悩みと店舗の強みを掛け合わせたテーマを1行で決める
  例：「新生活前の髪質改善で、毎朝のスタイリング時間を半分にする」

STEP3: 以下の構造で投稿を生成する
  フック   → 選んだお悩みへの共感から始める（施術名からは始めない）
  ボディ   → 季節の理由づけ＋サロンの解決策を自然につなげる
  CTA      → 「今だからこそ来店すべき理由」を一言添えて予約へ誘導

STEP4: Instagramハッシュタグを3層構造で15個生成する
  ビッグワード  3個（#ヘアカラー #髪質改善 #美容院 等）
  ミドルワード  7個（施術名・お悩み・ターゲット層に紐づくもの）
  スモールワード 5個（地域名＋施術・季節タグを必ず2個含める）

読んだ人が「今すぐ予約したい」と感じる投稿を作ること。
`;
...
4. Instagramハッシュタグは3層構造で15個生成する
   - ビッグワード  3個（#ヘアカラー #髪質改善 #美容院 等）
   - ミドルワード  7個（施術名・お悩みに紐づくタグ）
   - スモールワード 5個（地域名＋サロン名＋季節タグ必須）
`;
        } else if (patternId === "B") {
...
4. 締めは以下の流れで自然にサロンへ誘導する
   ...
   →「プロフィールのリンクからカウンセリングのご予約をどうぞ」

5. 最後に必ず以下を入れる
   「💾 保存して後で見返してください」

6. Instagramハッシュタグは3層構造で15個生成する
   ビッグワード  3個（#ヘアケア #美容院 #髪質改善 等
                      100万投稿以上の汎用タグ）
   ミドルワード  7個（テーマに紐づく具体的なタグ
                      例：#ヘアケア方法 #正しいシャンプー
                          #髪のお悩み #くせ毛ケア 等）
   スモールワード 5個（地域名＋サロン名＋季節タグ必須
                      例：#長野美容院 #飯綱町美容室
...
`;
        ...
        let ctaToAppend = "";
        if (ct === "phone") ctaToAppend = (cv || si?.phone) ? `ご予約・お問い合わせはお電話で：${cv || si?.phone}` : "";
        else if (ct === "reservation" && cv) ctaToAppend = `ご予約はこちらから：${cv}`;
        else if (ct === "line") ctaToAppend = (cv || lineUrl) ? `ご予約・お問い合わせはプロフィールのLINEからお待ちしております（${cv || lineUrl}）` : "";
        else if (ct === "other" && cv) ctaToAppend = cv;
        else if (cv && (/^https?:\/\//i.test(cv) || cv.includes("line.me"))) ctaToAppend = `ご予約・お問い合わせはプロフィールのLINEからお待ちしております（${cv}）`;
        ...
        if (!ctaToAppend) ctaToAppend = "ご予約・お問い合わせはプロフィールのリンクからお気軽にどうぞ。";
